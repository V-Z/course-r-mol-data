...
echo
rsync -arv --exclude=*~ --progress rcourse laddobar:~/soubory.trapa.cz/htdocs/ || { echo "Error!" && exit 1; }
echo
...
if [ "$1" == "all" ]; then
	...
	echo
	rsync -av --progress presentation/r_mol_data_phylogen.pdf laddobar:~/soubory.trapa.cz/htdocs/rcourse/ || { echo "Error!" && exit 1; }
	echo
	# Data
