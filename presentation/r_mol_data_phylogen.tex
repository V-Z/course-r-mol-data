...
\begin{frame}{Brief overview of molecular data types}
  \begin{itemize}
    \item Fragmentation data -- length polymorphism
    \begin{itemize}
      \item Codominant data -- e.g. microsatellites (SSRs)
      \begin{itemize}
	\item Variability in number of short (usually 1-3~bp) oligonucleotide repeats (ATAT vs. ATATAT, typically ca. 50-250 repeats)
	\item Very variable, fast evolving
	\item Mainly for population genetics, relationships among closely related species
      \end{itemize}
      \item Presence/absence data -- AFLP, RFLP,~\ldots
      \begin{itemize}
	\item The allele is or is not presented  -- it is impossible to distinguish heterozygots from dominant homozygots
	\item AFLP is very variable, but nowadays bit expensive and outdated
      \end{itemize}
    \end{itemize}
    \item Nucleic acid sequences (in nearly any form)
    \begin{itemize}
      \item Protein or DNA/RNA
      \item From ``classical'' Sanger sequencing -- long individual reads
      \item From modern HTS (NGS) -- 454 pyrosequencing, Illumina,~\ldots
      \item Whole sequences or SNPs (Single Nucleotide Polymorphism -- only polymorphic sites are retained
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Plot of Mantel Correlogram Analysis}
  ...
  Correlation (genetic similarity) in several distance classes (positive [up] in short distance [left], negative [down] in long [right]; [full] -- significant, [empty] -- not significant) -- see \texttt{?mantel.correlog} for details of settings of distance classes
\end{frame}
