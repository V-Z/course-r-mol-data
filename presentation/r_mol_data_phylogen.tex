...
\begin{frame}{Popular object classes (we are going to use) I} % TODO add more (like vcf)
  \begin{itemize}
...
\begin{frame}{Conversions among data types I} % TODO add more (like vcf)
  \begin{tabular}{llll}
...
\begin{frame}[fragile]{Install needed packages} % FIXME update package list: add adegraphics, mapplots, philentropy, StAMPP, stringi, vcfR
  \begin{spluscode}
    # Simplest usage
    install.packages("PackageName") # Case sensitive!
...
\begin{frame}[fragile]{Install packages without setting the repositories} % FIXME update package list
  \begin{itemize}
...
\begin{frame}[allowframebreaks]{Our data\ldots}
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Training data from packages \textbf{Adegenet} (sequence of influenza from USA sampled in several years; genotypes of cattle breeds, \texttt{?adegenet::microbov}; and \textit{Rupicapra rupicapra} (Bovidae) genotypes from French Bauges mountains, \texttt{?adegenet::rupica}), \textbf{ape} (morphological traits of Carnivora, \texttt{?ape::carnivora}) and \textbf{caper} (phylogeny and morphological traits of shorebirds, \texttt{?cape::shorebird})
    \end{itemize}
...
\begin{frame}[allowframebreaks]{Input/output data formats}{Representative selection} % TODO Input/output data formats
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Common format to store data
    ...
    \begin{itemize}
      \item Simple and populat text format to store genetic sequences (DNA, RNA, proteins)
      \item Line 1 of every records starts with \texttt{>} and contain name/description of the sequence (\texttt{> Seq 1}), line 2+ contains the sequence (\texttt{ATCG\ldots}) --- until end of file or next \texttt{>}
    \end{itemize}
    \item \href{https://en.wikipedia.org/wiki/FASTQ_format}{\textbf{FASTQ}}
    \begin{itemize}
      \item Text based format to store sequences (mainly nucleotide)
      \item Every record consists of 4 lines: sequence ID (with possible description) starting with \texttt{@}, the sequence (\texttt{ATCG\ldots}), \texttt{+} optionally follwoed by the same ID as line 1, and quality values for nucleotides from line 2
...
\begin{frame}[fragile]{Reading VCF}
  \begin{itemize}
    \item Download input file from \url{https://soubory.trapa.cz/rcourse/arabidopsis.vcf.gz} (\texttt{read.vcfR} does not work well with network addresses)
...
\begin{frame}[fragile]{Number of missing values in each locus}
  ...
  \end{spluscode}
  \begin{itemize}
    \item Those tools are designed mainly for situation when having multiple (nearly) complete genomes --- not needed for smaller datasets
