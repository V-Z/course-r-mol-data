...
% \usepackage{fixltx2e} % fixltx2e is not required with released after 2015 (fixltx2e), all files are now in the LaTeX kernel
\usepackage{metalogo}
\usepackage{xunicode}
\usepackage{fontspec}
...
\begin{itemize}
 ...
 \item Tinn-R (Windows only) \url{https://sourceforge.net/projects/tinn-r/}, \url{http://nbcgib.uesc.br/lec/software/editores/tinn-r/en}
...
\begin{itemize}
 ...
 \item Packages commonly have to be compiled -- R will do it automatically, but install basic Linux packages for building
...
\begin{frame}{Linux -- Debian/Ubuntu}
\begin{itemize}
 ...
 \item As \texttt{<my.favorite.cran.mirror>} select \alert{\texttt{r.adu.org.za}}, see \alert{CRAN Mirrors} at \url{https://cran.r-project.org/}
\end{itemize}
\end{frame}
...
\begin{itemize}
 ...
 \item Add repositories for appropriate version of your distribution
 \begin{itemize}
  \begin{footnotesize}
...
\begin{itemize}
 ...
 \item Object with data is on the left: ``\texttt{<-}'' says to insert result of the function \texttt{SomeFunction} on the right into the object \texttt{ModifiedObject} on the left
...
\begin{itemize}
 ...
 \item R saves history of commands into file \texttt{.Rhistory} file within working directory (by default hidden in Linux/Mac)
...
\begin{frame}[fragile, label=repos]{Set repositories}
  \begin{spluscode}
    ...
    # We will need extra repositories. For Bioconductor keep same version
    # as is version of your R installation (3.3 in this case).
    options(repos=c("http://cran.at.r-project.org",
      "http://r-forge.r-project.org/", "http://www.rforge.net/",
      "https://bioconductor.statistik.tu-dortmund.de/packages/3.3/bioc",
      "https://bioconductor.statistik.tu-dortmund.de/packages/3.3/data/
      annotation", "https://bioconductor.statistik.tu-dortmund.de/
      packages/3.3/data/experiment",
      "https://bioconductor.statistik.tu-dortmund.de/packages/3.3/extra"))
...
\begin{frame}[fragile]{Bioconductor and others -- differences from another repositories}
  \begin{spluscode}
    ...
    install.pacakges(pkgs=c("adegenet", "poppr", "phytools"))
...
\begin{frame}{Note about paths to import the data}
\begin{itemize}
 \item Generally, R can accept any local or web location
 \item Web locations start with \texttt{https://}, \texttt{http://} or \texttt{ftp://}
 \item Local paths (within one computer) can be absolute or relative
 \begin{itemize}
  \item Absolute paths start from the top of files hierarchy: on UNIX (Linux, Mac OS~X, \ldots) it use to look like \texttt{/home/USER/}, on Windows like \texttt{C:\textbackslash}
  \item Relative paths start in current directory (so \textbf{no} with \texttt{/} or \texttt{C:}) -- in the easiest case the input file is in same directory as is R's working directory -- verify by \texttt{getwd()} and \texttt{dir()} -- you then need to specify only the filename
  \item If loading data from computer, carefully check the paths or use function \texttt{file.choose()} to interactively pick up the file anywhere in the computer -- it can replace any filename parameter
 \end{itemize}
 \item Some R functions have problems with spaces and special (non-alphanumeric and accented) characters -- try to avoid them
 \item One of the most commons source of errors -- when the command fails, double check paths (and Internet connection, if applicable)
\end{itemize}
\end{frame}
...
\end{itemize}
As for AFLP, we need two files: the data matrix and individual's populations (it can be combined in one files -- next slide)\\
...
\begin{itemize}
 ...
 \item Genind object (since Adegenet 2) is able to store mixed ploidy data, but not all analysis are able to handle that\ldots
\end{itemize}
\end{frame}

\subsection{DNA sequences, SNP}

\begin{frame}[fragile]{Import of DNA data I}
  \begin{spluscode}
...
\begin{frame}[fragile]{Import of DNA data II}
  \begin{spluscode}
...
\begin{multicols}{2}
  ...
  \end{spluscode}
  \vfill
  This is very basic function to make dendrogram. There are better possibilities (NJ etc -- see further).
\end{multicols}
\end{frame}
...
\begin{frame}{Plot of Mantel Correlogram Analysis}
  ...
  Correlation (genetic similarity) in several distance classes (positive in short distance, negative in long) -- see \texttt{?mantel.correlog} for details of settings of distance classes
\end{frame}
...
\begin{frame}[allowframebreaks]{Our packages\ldots}{Packages we used as well as we didn't, but they can be very useful}
  \textbf{We used following packages} -- but not all functions -- explore them for more possibilities
  \begin{itemize}
    \item adegenet
    \item adephylo
    \item ade4
    ...
    \item vegan
  \end{itemize}
  \textbf{Another interesting packages} -- explore them yourselves
  \begin{itemize}
    \item apex
    \item corHMM
    \item diveRsity
...
% Arlequin
% Linarius
% https://cran.r-project.org/web/packages/polyfreqs/
...
% Jaccard index
% https://github.com/thibautjombart/apex https://cran.r-project.org/package=apex
