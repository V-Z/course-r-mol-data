...
\begin{frame}[fragile, label=repos]{Set repositories}
  \begin{spluscode}
    ...
    options(repos=c("http://cran.at.r-project.org",
      ...
      "https://bioconductor.statistik.tu-dortmund.de/packages/3.2/bioc",
      "https://bioconductor.statistik.tu-dortmund.de/packages/3.2/data/
      annotation", "https://bioconductor.statistik.tu-dortmund.de/
      packages/3.2/data/experiment",
      "https://bioconductor.statistik.tu-dortmund.de/packages/3.2/extra"))
...
\begin{frame}[fragile]{Install packages without setting the repositories}
  \begin{itemize}
   \item If repositories from slide~\ref{repos} are not set (for any reason), it is possible to install in several steps packages from main repository and from another sources (following slides)
  \end{itemize}
  \begin{spluscode}
...
\begin{frame}[fragile]{Install phyloch package}{Example of installation of package not available in any repository}
  ...
  \begin{spluscode}
    ...
    install.packages(pkgs=c("ape", "colorspace", "XML"),
      dependencies=TRUE)
    # It is possible to specify direct path (local or web URL)
    # to package source
...
\begin{frame}[fragile]{Bioconductor and others -- differences from another repositories}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item Although Bioconductor developers recommend usage of \texttt{biocLite}\ldots
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Our data\ldots}
  ...
  \begin{block}{Always save your work!}
    \alert{We will use data objects for the whole course -- all the time save your workspace!} Use possibilities of your GUI or \texttt{save}/\texttt{load} functions.
  \end{block}
\end{frame}
...
\begin{frame}[fragile]{Another data manipulation}
  \begin{itemize}
    \item SNPs can be into genind imported in same way as AFLP
  \end{itemize}
  \begin{spluscode}
...
\begin{itemize}
 ...
 \item Do not use underscores (``\_'') or minuses to name objects in R -- only numbers, letters or dots
...
\begin{frame}[fragile]{Import sequences from GenBank}
  \begin{spluscode}
    ...
    usflu.dna3 <- read.fasta(file="http://adegenet.r-forge.r-project.org/
...
\begin{frame}[fragile]{AMOVA}
  ...
  Another possibility is \texttt{poppr.amova} -- for more complicated hierarchy, see \texttt{?poppr.amova}
\end{frame}
...
\begin{multicols}{2}
  \includegraphics[height=5.5cm]{kmeans.png}
  \begin{spluscode}
    # See results as text
...
\end{itemize}
  \begin{spluscode}
    data(rupica) # Loads adegenet's training dataset
                 # Rupica rupica from French Alps
...
\begin{frame}[fragile]{Produce maps of respective inferred clusters}
  ...
  \end{spluscode}
  \begin{itemize}
    \item Maps are produced as PS (PostScript) files
...
% polysat
% splits
% coalescence based approach for creation of species tree from multiple gene trees -- \href{https://faculty.franklin.uga.edu/lliu/content/phybase}{phybase}
