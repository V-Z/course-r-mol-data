...
% # Compare topology of the species trees
% all.equal.phylo(tree.sp.min, tree.sp.mean, use.edge.length=FALSE, use.tip.label=TRUE, index.return=FALSE)



%%%

% # Robinsons-Foulds distance - some trees are not fully bifurcating - some results might be misleading
% trees.nexus.d.rf <- RF.dist(tree1=trees.nexus, tree2=NULL, check.labels=TRUE)
% # trees.nexus.d.rf <- multiRF(trees.nexus)



%%%

% # PCoA of trees distances
% trees.d.raxml.red.pcoa <- dudi.pco(d=quasieuclid(distmat=as.dist(m=trees.d.raxml.red, diag=TRUE, upper=TRUE)), full=TRUE)
...
% # Plot multiple trees in once
% # Comparing MrBayes and RAxML
% #par(mfrow=c(3, 2))
% cophyloplot(x=ladderize(tree.sp.astral), y=ladderize(tree.sp.raxml.astral), assoc=tips.labels, use.edge.length=FALSE, space=60, length.line=1, gap=2, type="phylogram", rotate=TRUE, col="red", lwd=1.5, lty=2)
% title("ASTRAL trees (MrBayes left and RAxML right)")
% legend("topleft", legend="A", cex=2, bty="n", x.intersp=-1, y.intersp=-2.5)
